...
const CardView = ({ item, totalPrice, setTotalPrice }) => {
  ...
  const handleQuentityValue = (e) => {
    e.target.value < 0
      ? alert("nothind add nagative value ")
    ...
    <li className="6 flex sm:flex-row flex-col md:flex-col lg:flex-row  justify-between mb-10 bg-secondary_white p-3 shadow-md">
      <div className="flex flex-col sm:flex-row md:flex-col lg:flex-row ">
        ...
        <div>
          ...
          <br />
          <span className="flex items-center text-light_tomato">
            ${price} <BiBadgeCheck />{" "}
          </span>
      ...
      <div>
        <button
          onClick={() => handleRemoveFormCard(item.id)}
          className="text-light_tomato hover:underline"
        >
          Remove
        </button>
        <br />
        ...
        <input
          value={quentityValue}
          onChange={handleQuentityValue}
          type="number"
          className="w-20 text-center   bg-dark_blue text-white"
        />
      </div>
    </li>
  );
